...
const AddEventModal = (props) => {
  ...
  React.useEffect(() => {
    ...
    let newSlot = {
      start: convertFrom24To12Format(String(props.selectedSlot.start).slice(16, 21)),
      end: convertFrom24To12Format(String(props.selectedSlot.end).slice(16, 21)),
  ...
  function verifyFields() {
    ...
    slots.forEach((slot) => {
      let startTime = new Date(2022, 1, 1, convertTime12to24(slot.start).split(":")[0], convertTime12to24(slot.start).split(":")[1]);

      let endTime = new Date(2022, 1, 1, convertTime12to24(slot.end).split(":")[0], convertTime12to24(slot.end).split(":")[1]);
      ...
      if (slot.start === "" || slot.end === "") {
        ...
        status = false;
        toastShown = true;
      } else if (((endTime - startTime) / 1000 / 60 < 30 || (endTime - startTime) / 1000 / 60 > 60) && (endTime - startTime) / 1000 / 60 !== -660) {
        if (!toastShown) toast.warn("Invalid Slot Time, slot diff should be 30 or 60 min");
        status = false;
        toastShown = true;
      }

      if (slot.start.split(" ")[1] === "am" && slot.end.split(" ")[1] !== "am") {
        if (!toastShown) toast.warn("Invalid Slot Time, slot diff should be 30 or 60 min");
        status = false;
        toastShown = true;
      } else if (slot.start.split(" ")[1] === "pm" && slot.end.split(" ")[1] !== "pm") {
        if (!toastShown) toast.warn("Invalid Slot Time, slot diff should be 30 or 60 min");
        status = false;
        toastShown = true;
      }
    ...
    if (props.selectedDateSlots.length) {
      ...
      props.selectedDateSlots.forEach(async (date) => {
        ...
        body.availability.slots = body.availability.slots.map((slot) => {
          ...
          let end = new Date(
            body.availability.date.setHours(parseInt(convertTime12to24(slot.end).split(":")[0]), parseInt(convertTime12to24(slot.end).split(":")[1]))
          );
      ...
      <div className={modalStyles.modalBackdrop}>
        <div className={modalStyles.modal}>
          ...
          <div className={modalStyles.slots}>
            {slots.map((slot, i) => {
              return <Slot slot={slot} i={i} handleChange={handleChange} deleteSlot={deleteSlot} reschedule={props.reschedule} />;
            })}
          </div>
