...
const CreateCourseModal = (props) => {
  ...
  const [formValues, setFormValues] = useState({ courseImage: null, language: "English", course: "Academics" });
  ...
  const handleChange = (e) => {
    //  console.log(e);
    if ((e.target.name === 'price' || e.target.name === 'price1' || e.target.name === 'price2') && e.target.value <= 0) {
  ...
  const handleSubmit = async () => {
    ...
    else if (formValues.courseImage.type !== 'image/png' && formValues.courseImage.type !== 'image/jpg' && formValues.courseImage.type !== 'image/png' && formValues.courseImage.type !== 'image/jpeg') {
    ...
    if (result.msg === "Course Created Successfully") {
      ...
      setFormValues({ courseImage: null, language: "English", course: "Academics" });
      setPage(1);
                  ...
                  <form className={styles.createCourseForm1}>
                    ...
                    >
                      ...
                      {Object.keys(languages).map((language) => {
                        return (
                          <>
                            <option value={language} selected={formValues.language === language ? true : false}>
                              {language}
                            </option>
                          </>
                        );
                    ...
                    >
                      ...
                      <option value="Academics" selected={formValues.course === "Academics" ? true : false}>
                        Academics
                      </option>
                      <option value="Spoken Languages" selected={formValues.course === "Spoken Languages" ? true : false}>
                        Spoken Languages
                      </option>
                      <option value="Test Preparation" selected={formValues.course === "Test Preparation" ? true : false}>
                        Test Preparation
                      </option>
                    </select>
                    <select
              ...
              {page === 2 ? (
                <>
                  <form className={styles.createCourseForm2}>
                    <input onChange={handleChange} type="number" placeholder="1 Lesson Price" name="price" value={formValues.price ? formValues.price : ""} />
                    <input onChange={handleChange} type="number" placeholder="5 Lesson Price" name="price1" value={formValues.price1 ? formValues.price1 : ""} />
                    <input onChange={handleChange} type="number" placeholder="10 Lesson Price" name="price2" value={formValues.price2 ? formValues.price2 : ""} />
