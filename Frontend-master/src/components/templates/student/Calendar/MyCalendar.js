...
function MyCalendar(props) {
  ...
  const bookSlot = (event) => {
    ...
    history.push("/payment", { course: props.course, teacherData: props.teacherData, event, trail:false });
  };
        ...
        <div className={modalStyles.modal}>
          ...
          <div className="popup" style={{ textAlign: 'center', marginTop: '20px' }}>
            <div className="header">
              <img src={props.teacherData.teacherProfilePic.data} alt="" style={{ width: '100px' }} />
            ...
            <div className="popup-body">
              ...
              <h5>
                Time:{" "}
                <span>
                  {convertFrom24To12Format(String(eventPopup.start).slice(16, 21)) + "-" + convertFrom24To12Format(String(eventPopup.end).slice(16, 21))}
                </span>
              </h5>
            </div>
  ...
  function Event({ event }) {
    ...
    return (
      ...
      <div className="event">
        <p>{event.title}</p>
        <div id="book-lesson" className={`BookLessonHoverCard popover top model-${popupModal === Math.round(event.start.getTime() / 1000) ? 'show' : 'hide'}`}>
          <div className="arrow" style={{ left: "50%" }}></div>
          <div className="popover-content"><header class="flex flex-direction-row flex-align-center">
            <img src={props.teacherData.teacherProfilePic.data} className="ProfilePic verb-profile-pic img-responsive round small" alt="Profile Picture" style={{ marginRight: "5px" }} />
            <div>
              <strong><font ><font >{props.teacherData.firstName.data + " " + props.teacherData.lastName.data}</font></font></strong>
            </div>
          </header>
            <div className="BookLessonHoverCard--body flex flex-direction-column flex-align-center">
              <div className="flex flex-direction-column">
                ...
                <div className="flex flex-direction-row"><strong>Time:</strong>
                  <div>{convertFrom24To12Format(String(event.start).slice(16, 21)) + "-" + convertFrom24To12Format(String(event.end).slice(16, 21))}
                  </div>
                </div>
              </div>
              <button type="button" class="btn btn-default btn-block" onClick={() => {
                ...
                Book Now</button></div><div style={{ marginBottom: "-12px" }}>
            </div>
          </div>
        </div>
    ...
    return (
      <div className="rbc-toolbar">
        <span className="rbc-btn-group">
          <button type="button" class="prev" onClick={goToBack}>Prev</button>
        </span>
        <span className="rbc-toolbar-label">{toolbar.label}</span>
        <span className="rbc-btn-group">
          <button type="button" class="next" onClick={goToNext}>Next</button>
        </span>
      </div>
    )
  }
  ...
  MyWeek.title = (date, { localizer }) => {
    ...
    return `${monthNames[date.getMonth()]} ${date.getDate()} - ${monthNames[end.getMonth()]} ${end.getDate()}`;
  }
  ...
  return (
    <>
      <div
        style={{ marginTop: "10px", borderRadius: "10px", width: width >= 992 ? "100%" : "90%", backgroundColor: "#fefeff", padding: "20px" }}
